...
import { useAllBuysQuery, useBuyPropertyMutation } from '@/redux/features/buyProperty/buy.property.api';
...
const BuyProperty = ({ property }: { property: TListing }) => {
    ...
    const isBrought = buys?.find((buy: { refId: string }) => buy.refId == property._id)
        ...
        <div>
            ...
            <div className='border p-2 rounded-md mt-6 bg-gray-50'>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <p className='bg-red-500 text-white p-1 rounded-md text-sm font-medium'>For {property.for}</p>
                </div>
                ...
                {user?.role == "buyer" && <button disabled={isBrought !== undefined} onClick={() => handleMakeBuyProposal(property)} className='bg-green-600 py-2 rounded-md text-white font-medium w-full mt-4 hover:bg-green-700'>{isBrought == undefined ? "Make Buy Proposal" : "Alreadry in process"}</button>}

            </div>
            <div className='flex flex-col items-center mt-6'>
                <h1 className='font-medium'>Have a question? Get in touch!</h1>
                <Link href="/contactUs"><button className='text-green-600 py-2 px-6 rounded-md hover:text-white font-medium mt-4 hover:bg-green-600 hover:text-white border-green-600 border'>Contact Us</button></Link>
            </div>
        </div>
    );
};
