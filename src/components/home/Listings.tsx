        ...
        <div className='px-4 md:px-24'>
            ...
            <div>
                <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
                    {listings?.slice(0, 4).map((listing: TListing) => <Card key={listing._id} listing={listing}></Card>)}
                </div>
                <Link className='flex justify-center' href='/listings'><button className='bg-orange-400 mt-5 font-medium p-2 text-white rounded-md'>View All Listings</button></Link>
            </div>
        </div>
    );
};
