...
const SingleListing = ({ params }: { params: { listing: string } }) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:5000/api/v1/listing/${params.listing}`)
            .then(response => {
    ...
    return (
        <div className='pt-16'>
            <div
                className="h-[540px] bg-cover bg-center grid grid-cols-2 items-center gap-24"
                style={{ backgroundImage: `url(${listings?.image?.imageUrl || imagse.src})` }}
            >
                <div className='bg-black bg-opacity-50 p-12 text-white ml-12 rounded-lg'>
                    <div className='flex justify-between items-center gap-24'>
                        <h1 className='font-medium text-xl'>{listings.title}</h1>
                        <h1 className='text-red-500 text-xl font-medium'>${listings.price}</h1>
                    </div>
